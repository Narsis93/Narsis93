
## 👋 Hi! I'm Narges
I am a QA Engineer and Web Developer passionate about **software testing, automation, and building clean and reliable systems**.

---

## 🔧 Skills
-QA Manual & Automation

-Test Planning / Test Case Design

-Selenium / Playwright

-API Testing (Postman, RestAssured)

-JavaScript / Python

-Git & GitHub

---

## 📌 Featured Projects
### 🟣 **Login Automation Test**
- Automated login flow with valid and invalid user scenarios
- Built using Selenium / Playwright

### 🟣 **Inventory API Testing**
- API testing for retrieving product inventory
- Includes success and error response validation

### 🟣 **Add to Cart Test Flow**
- End-to-end test for adding items to the cart
- Validates item state, quantity, and UI behavior

### 🟣 **Cart View Validation**
- Checks accuracy of cart items and display behavior

---
![Narges's GitHub stats](https://github-readme-stats.vercel.app/api?username=Narsis93&show_icons=true&theme=default)

---
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Narsis93&layout=compact)
---
![GitHub Trophies](https://github-profile-trophy.vercel.app/?username=Narsis93&theme=flat)
---

## 📬 Contact
- GitHub: https://github.com/Narsis93
- Email: soltaninarges71@gmail.com
- LinkedIn: https://www.linkedin.com/in/narges-soltani-b10394204?utm_source=share_via&utm_content=profile&utm_medium=member_android
